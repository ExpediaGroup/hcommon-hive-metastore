...
import com.hotels.hcommon.hive.metastore.core.HiveEntityFactory;
import com.hotels.hcommon.hive.metastore.core.util.LocationUtils;
...
public class LocationUtilsTest {
  ...
  private Table createTable() {
    return HiveEntityFactory.newTable("name", "db", Lists.<FieldSchema> newArrayList(), HiveEntityFactory.newStorageDescriptor(new File("/a/b%c")));
  }

  private Partition createPartition() {
    Table table = createTable();
    Partition partition = HiveEntityFactory.newPartition(table);
    partition.setSd(HiveEntityFactory.newStorageDescriptor(new File("/a/b%c/x=y")));
    return partition;
  }
}
